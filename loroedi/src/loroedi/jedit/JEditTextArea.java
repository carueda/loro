...
public class JEditTextArea extends JComponent
	implements util.jterm.ITextArea  //carueda 10/09/01
{
	/**
	...
	public int _offsetToX(int line, int offset)
	{
		...
		else
		{
			...
			// carueda: 2002-02-10
			// quito la siguiente linea porque esa variable
			// toolkit no se utiliza para nada:
			...
			for(;;)
			{
				...
				else
				{
					...
					// que no arreglaba la posicion del caret al
					// cambiar dinamicamente el Font.
					// El painter sabe como ajustar el FontMetrics, 
					// pero NO era usado para esto:
					// Asi que aqui lo uso, y voila!, posicion
					// del caret ahora esta correcto!
	...
	public final void getText(int start, int len, Segment segment)
	{
		...
		{
			// Siguiente linea comentada por carueda: la excepcion se esta presentando
			// cuando se limpia el area del interprete. Supongo que se trata de
			// un problema de sincronizacion mio (quiza).
			// Slava lo tenia imprimiendo el stack-trace, pero aparentemente
			// la linea a continuacion pone las cosas consistentes.

			//bl.printStackTrace();             // comentado por carueda

			segment.offset = segment.count = 0; // <<-- original de Slava
		}
	}
	/**
...
//		}
	...
	public void setText(String text)
	{
		try
		{
			...
			// carueda 2001-02-09
			// Lo siguiente corrige el comportamiento no deseado
			// de no manejar bien los tokens que cubren varias lineas,
			// concretamente cuando el inicio de tal token esta mas
			// arriba de la parte visible --se troca el highlight.
			// Esto lo detecte con los literales cadena, pero tambien
			// sucedia con los comentarios /* ... */
			// Como aqui se esta poniendo el texto completo,
			// entonces se provoca el procesamiento de tokens
			// de todo el documento:
			TokenMarker tm = getTokenMarker();
			if ( tm.supportsMultilineTokens() )
				setTokenMarker(tm);
		}
		catch(BadLocationException bl)
		{
	...
	/**
	 * carueda 10/09/01.
	 * Only because jterm.ITextArea requieres it, and I don't want
	 * to make a subclass only for this.
	 */
